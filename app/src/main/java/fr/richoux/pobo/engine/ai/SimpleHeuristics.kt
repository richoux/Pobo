...
class SimpleHeuristics(color: Color) : AI(color) {
  ...
  private fun check_three_in_a_row(
    from_row: Int,
    from_col: Int,
    direction: Direction,
    type: PieceType
  ...
  private fun check_two_in_a_row(
    from_row: Int,
    from_col: Int,
    direction: Direction,
    type: PieceType
    ...
    return when(type) {
      ...
      PieceType.PO -> _simulation_grid[index] == _simulation_grid[next_index] && abs(
        _simulation_grid[index].toInt()
      ) == 1
      else -> _simulation_grid[index] == _simulation_grid[next_index] && abs(_simulation_grid[index].toInt()) == 2
    }
  }

  private fun count_Po_in_a_row(
    from_row: Int,
    from_col: Int,
    direction: Direction
  ): Int {
  ...
  private fun compute_partial_score(
    from_row: Int,
    from_col: Int,
    direction: Direction
  ): Pair<Int, Int> {
    ...
    if(check_three_in_a_row(from_row, from_col, direction, PieceType.BO)) {
      ...
      if(check_three_in_a_row(from_row, from_col, direction, PieceType.PO)) {
        ...
        if(check_three_in_a_row(from_row, from_col, direction, PieceType.WHATEVER)) {
          score += count_Po_in_a_row(
            from_row,
            from_col,
            direction
          ) * (if(is_player_piece) 10 else -10)
          jump_forward = 1
        } else {
